...
int main(void)
{
	...
	checktree =
	    newtCheckboxTreeMulti(-1, -1, 10, " ab", NEWT_FLAG_SCROLL);
	...
	newtCheckboxTreeAddItem(checktree, "Really really long thing",
				(void *) 3, 0, NEWT_ARG_APPEND,
				NEWT_ARG_LAST);
	newtCheckboxTreeAddItem(checktree, "number 5", (void *) 5,
				NEWT_FLAG_SELECTED, NEWT_ARG_APPEND,
				NEWT_ARG_LAST);
	...
	newtGridSetField(grid, 0, 0, NEWT_GRID_COMPONENT, checktree, 0, 0,
			 0, 1, NEWT_ANCHOR_RIGHT, 0);
	newtGridSetField(grid, 0, 1, NEWT_GRID_COMPONENT, button, 0, 0, 0,
			 0, 0, 0);
	...
	result =
	    newtCheckboxTreeGetMultiSelection(checktree, &numselected,
					      'b');
	ptr = result;
	if (!result || !numselected)
		printf("none selected\n");
